...
- (NSImage*)image{
    if(self.powerSource.charged){
    ...
    }else if(self.powerSource.charging){
        ...
        const CGFloat   capBarLeftOffset    = 3.0f;
        CGFloat         capBarHeight        = batteryLevelLeft.size.height;
        const CGFloat   capBarTopOffset     = 3.0f;
        CGFloat         capBarLength        = ceil([self.powerSource.remainingChargeInPercent integerValue] / 8.0f);
        if (capBarLength <= 2) {
            capBarLength = 2 + 0.1f;
        }
