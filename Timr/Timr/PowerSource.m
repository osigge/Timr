...
- (void)initializePowerSourceFromCurrentSource;
...
@synthesize current, capacity, cycleCount, watt, temperature, remainingChargeInPercent, source;

@synthesize charged, charging, calculating, finishingCharge, lowBatteryWarning, batteryPower;

- (id)init{
    self = [super init];
    if (self) {
        [self initializePowerSourceFromCurrentSource];
    }
    return self;
}
...
- (NSString*)stringWithHumanReadableTimeRemaining{
    if(self.charged) return NSLocalizedString(@"Charged", @"Charged notification");
    if(self.calculating) return NSLocalizedString(@"Calculating…", @"Calculating sidetext");
